...
def getBattingStats(teamId, year):
    subquery = (
        db.session.query(
            ...
            func.div(func.sum(Batting.b_BB), func.sum(Batting.b_AB)).label("BB%"),
            func.div(func.sum(Batting.b_SO), func.sum(Batting.b_AB)).label("K%"),
            func.div(
                func.sum(Batting.b_2B) + (2 * func.sum(Batting.b_3B)) + (3 * func.sum(Batting.b_HR)),
                func.sum(Batting.b_AB)).label("ISO"),
            func.div(func.sum(Batting.b_H) - func.sum(Batting.b_HR),
                     func.sum(Batting.b_AB) - (func.div(func.sum(Batting.b_SO),
                                                        func.sum(Batting.b_AB))) - func.sum(
                         Batting.b_HR) + func.sum(Batting.b_SF)).label("BABIP"),
            func.div(
