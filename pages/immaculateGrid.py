    ...
    if form.validate_on_submit():
        ...
        for i in range(3):  # 3 rows
            for j in range(3):  # 3 columns
                question = questions[j] + " & " + questions[i + 3]
                solution = solutionNames[i * 3 + j]

                query = Queries(user_ID=current_user.user_ID, q_QUESTIONS=question, q_SOLUTIONS=solution)
...
def scrapeImmaculateGridQuestions(url):
    questions = ["Detroit Tigers","Only One Team","≤ 3.00 ERA Career","Gold Glove", "Played First Base min. 1 game", "Houston Astros"]
    return questions
